...
impl Session {
    ...
    pub(crate) fn new(
        ...
        } else if target.architecture() == Architecture::Sparc {
    ...
    pub fn get_arm_interface(&mut self) -> Result<&mut dyn ArmDebugInterface, ArmError> {
        let interface = match &mut self.interfaces {
            ...
            ArchitectureInterface::Jtag(..) | ArchitectureInterface::SystemBus(..) => {
    ...
    pub fn get_riscv_interface(
        ...
        match &mut self.interfaces {
            ...
            ArchitectureInterface::Arm(_) | ArchitectureInterface::SystemBus(..) => {
                Err(RiscvError::NoRiscvTarget.into())
            }
        }
    }
    ...
    pub fn prepare_running_on_ram(
        ...
        match self.target.debug_sequence.clone() {
            ...
            DebugSequence::Leon3(leon3) => {
                leon3.prepare_running_on_ram(vector_table_addr, self, core_id)
            }
